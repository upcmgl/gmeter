...
typedef struct {
	...
	uint8_t allow_charge;//1 允许充电 0 不允许充电 NB发送数据时不能给超级电容充电	
	...
	INT32U edp_event_SZDCQY;       //时钟电池欠压事件
	INT32U edp_event_SDTSD;        //市电停上电事件
	INT32U edp_event_FZDYTSD;      //辅助电源停上电事件
	INT32U edp_event_YGGLFX;       //有功功率方向
	INT32U edp_event_WGGLFX;       //无功功率方向
	INT32U edp_event_AXDX;          //A相断相
	INT32U edp_event_BXDX;          //B相断相
	INT32U edp_event_CXDX;          //C相断相
#endif	
	INT8U  edp_warning_ack;
	INT8U  edp_rand_report_begin;   // 设置随机延后时间范围,开始时间
	INT8U  edp_rand_report_end;   // 设置随机延后时间范围,结束时间
	INT8U  edp_first_poweron;     //edp协议模块第一次上电
	INT8U  edp_second_connet;    //edp协议模块重新
#endif
#ifdef __HJ_212__
	INT8U passwd[7];  //
#endif 
