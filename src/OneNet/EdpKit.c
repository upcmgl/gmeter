...
//}
/* sava_data (C->S) */
EdpPacket* PacketSavedataJson2(const char* dst_devid,  char* data, int type, uint16 msg_id)
{
	EdpPacket* pkg = NULL;
	uint32 remainlen = 0;
	char* json_out = NULL;
	...
	remainlen = 1 + (msg_id ? 2 : 0) +              /* �̶�ѡ�� + ��Ϣ��־ */
		(dst_devid ? (2 + strlen(dst_devid)) : 0) + /* Ŀ���豸 */
		1 + (2 + json_len);                         /* �������� + Json������ */
...
//
///* 
 //* 定义3种JSON格式的打包和解包函数�? * 每种格式都支持int,double和char*三种类型
 //*/
...
char* req, uint32* req_len)
{
	...
	if (rc)
	return rc;
	if (ReadBytes(pkg, cmdid, *cmdid_len))
	return ERR_UNPACK_CMDREQ;

	rc = ReadUint32(pkg, req_len);
	if (rc)
	return rc;
	if (ReadBytes(pkg, req, *req_len))
	return ERR_UNPACK_CMDREQ;
...
//
    ...
    //remainlen = 1 + (msg_id ? 2:0) +              /* 固定选项 + 消息标志 */
        //(dst_devid ? (2+strlen(dst_devid)) : 0) + /* 目的设备 */
        //1 + 6 + (2+input_len);                        /* 数据类型 + 6字节时间+字符串长�?/
//
    //WriteRemainlen(pkg, remainlen);
//
	//
...
//
    //remainlen = 1 + (msg_id ? 2:0) +              /* 固定选项 + 消息标志 */
        //(dst_devid ? (2+strlen(dst_devid)) : 0) + /* 目的设备 */
        //1 + 2 +
        //6 + input_len;                        /* 数据类型 + 数据点个�?                                                     + 6字节时间+浮点数据�?/
    //WriteRemainlen(pkg, remainlen);
//
    //
