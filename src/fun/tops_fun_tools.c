...
FSFILE * usb_open_file(INT8U file_type, FSFILE *file)
{
    ...
    switch(file_type)
    {
	    case FILEFLAG_DEV_PROG:   //程序文件
        {
			...
			break;
		case FILEFLAG_LOG:
			filename ="0:G55_LOG.txt";
            break;
		case FILEFLAG_CONFIG:
			filename ="0:G55_CONFIG.txt";
			break;
		case FILEFLAG_OUT_FLASH:
			filename ="0:G55_OUT_FLASH.txt";
			break;
        default:
            filename=NULL;
            break;
...
if((file_type ==FILEFLAG_LOG) ||(file_type ==FILEFLAG_OUT_FLASH) )  
{
	ret = f_open_app(file, filename, FA_WRITE);
}
else
{
...
INT8U usb_output_log(INT8U file_type, FSFILE * file)
{
	FSFILE* usbFile;
	FRESULT res;
	UINT count =0;
	...
	index++;
//	INT8U data_buffer[512]={(index>>8)&0xFF,index&0xFF,0xaa,0xaa,0xaa,0xaa,0xaa,0xaa,0xaa};   //  UartObjDebug.send_ptr = data;
   // UartObjDebug.send_len = len;
   if(usb_log.read_pos!=usb_log.write_pos)
   {
	usbFile = usb_open_file(file_type, file);
	f_lseek(usbFile,usbFile->fsize);
	if( usb_log.read_pos>usb_log.write_pos)
	{
		f_write(usbFile, usb_log.buffer+usb_log.read_pos, (LOG_LEN-usb_log.read_pos)%LOG_LEN, &count);
		usb_log.read_pos = (usb_log.read_pos+count)%LOG_LEN;
	}
	else
	{
			res =f_write(usbFile, usb_log.buffer+usb_log.read_pos, (usb_log.write_pos-usb_log.read_pos)%LOG_LEN, &count);
			usb_log.read_pos = (usb_log.read_pos+count)%LOG_LEN;
	}
...
INT8U flash_buf[4096];
INT8U usb_output_flash_sector(INT8U file_type, FSFILE * file)
{
	FSFILE* usbFile;
	INT8U tmpData[50];
	FRESULT res;
	...
	usbFile = usb_open_file(file_type, file);
	f_lseek(usbFile,usbFile->fsize);
	if(output_flash_sector.sector_pos<output_flash_sector.begin_sector+output_flash_sector.read_len)
	{
		sprintf(tmpData, "===========%d===========", output_flash_sector.sector_pos);
		f_write(usbFile, tmpData, 50, &count);
		f_lseek(usbFile,usbFile->fsize);
		nor_flash_read_data(output_flash_sector.sector_pos,0,flash_buf,4096);
		res =f_write(usbFile, flash_buf, 4096, &count);
		output_flash_sector.sector_pos++;
	}
	if(output_flash_sector.sector_pos>= (output_flash_sector.begin_sector+output_flash_sector.read_len))
	{
...
INT8U usb_handle_config(INT8U file_type, FSFILE * file)
{
	FSFILE* usbFile;
	FRESULT res;
	UINT count =0;
	INT8U len;
	INT8U data_buffer[200]={0};
	INT16U  index =0,tmp=0,pos=0;
	
	usbFile = usb_open_file(file_type, file);
	f_read(usbFile, data_buffer, 200, &count);
	while(data_buffer[index++]!=0x23);// #FUN,1,BEGIN,10,LEN,10,#
	while(data_buffer[index++]!=0x2c)
	{
		//index++;
	NOP();
	}
	switch(data_buffer[index])
	{
		case 0x31:  //脚本功能1,导出flash部分sector；
			output_flash_sector.begin_sector=0;
			output_flash_sector.read_len=0;
			pos = str_find(data_buffer+index,count-index,"BEGIN,",6);
			index +=pos+6;
			while(data_buffer[index]!=',')
			{
				output_flash_sector.begin_sector = output_flash_sector.begin_sector*10+(data_buffer[index]-'0');
				index++;
			}
			output_flash_sector.sector_pos = output_flash_sector.begin_sector;
			pos = str_find(data_buffer+index,count-index,"LEN,",4);
			index +=pos+4;
			while(data_buffer[index]!=',')
			{
				output_flash_sector.read_len = output_flash_sector.read_len*10+(data_buffer[index]-'0');
				index++;
			}	
			output_flash_sector.flash_out_flag =1;		
			break;
		default:
			break;
	}
    ...
    //{
        ...
        //case 0:   //程序文件
                ...
                //{
                    ...
                    //reset_flag=usb_download_dat(FILEFLAG_LOG, &file);
				//}
				//break;
        //default:
                //break;
...
BOOLEAN usb_file_process(void)
{
    ...
    for(idx = 0; idx<2; idx++)
    {
        ...
        case 0:   //程序文件
                ...
                break;
        case 1:   //执行U盘脚本功能
			if(usb_file_exist(FILEFLAG_CONFIG, &file))
			{
				//record_sys_operate(OP_SYS_MAIN_UPDATE);
				usb_handle_config(FILEFLAG_CONFIG, &file);
			}
			break;				
        default:
                break;
...
BOOLEAN usb_file_process_log(void)
{
	...
	FSFILE file2;
	BOOLEAN  reset_flag=FALSE;
	static INT8U log_flag =0;
	for(idx = 0; idx<2; idx++)
	{
		switch(idx)
		{
			case 0:   //导出log功能。
				if(usb_file_exist(FILEFLAG_LOG, &file2))
				{
//#ifdef DEBUG
	//system_debug_info("===============out log state================");
//#endif			
				//record_sys_op erate(OP_SYS_MAIN_UPDATE);
					usb_output_log(FILEFLAG_LOG, &file2);
					log_flag ++;
					if( (log_flag&0x01) == 0x01)
					{
						//闪烁告警灯
						LED_comm_toggle();
					}
				...
				break;
			case 1:  //读U盘脚本文件的功能。格式#FUN,1,BEGIN,10,LEN,10,#功能1，从第10个块开始读10个块。
				if(output_flash_sector.flash_out_flag ==1)
				{
#ifdef DEBUG
		system_debug_info("===============out flash sector state================");
#endif					
					if(usb_file_exist(FILEFLAG_OUT_FLASH, &file2))
					{
#ifdef DEBUG
		system_debug_info("===============out flash sector state open file ok ================");
#endif							
						usb_output_flash_sector(FILEFLAG_OUT_FLASH, &file2);
						//log_flag ++;
						//if( (log_flag&0x01) == 0x01)
						{
							//闪烁告警灯
				...
				{
					if(usb_file_exist(FILEFLAG_OUT_FLASH, &file2)) //首先要存在导出flash的文件在。然后在常亮。
					{
						LED_green_on(1);
					}
