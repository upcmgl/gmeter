...
)
{
	...
	UINT i;
	DWORD b_vol, b_fat, b_dir, bdata;	/* LBA */
	...
	bdata = b_dir + n_dir;				/* Data area start sector */
	if (n_vol < bdata + au - b_vol) return FR_MKFS_ABORTED;	/* Too small volume */

	/* Align data start sector to erase block boundary (for flash memory media) */
	if (disk_ioctl(pdrv, GET_BLOCK_SIZE, &n) != RES_OK || !n || n > 32768) n = 1;
	n = (bdata + n - 1) & ~(n - 1);	/* Next nearest erase block from current data start */
	n = (n - bdata) / N_FATS;
